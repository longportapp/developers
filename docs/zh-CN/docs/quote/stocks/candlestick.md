...
longport kline TSLA.US
# Apple 周 K 线
longport kline AAPL.US --period week
# NVDA 最近 20 根日 K
longport kline NVDA.US --period day --count 20
</CliCommand>
...
```python
from longport.openapi import QuoteContext, Config, Period, AdjustType, TradeSessions, OAuthBuilder
...
```python
import asyncio
from longport.openapi import AsyncQuoteContext, Config, Period, AdjustType, TradeSessions, OAuthBuilder
...
```javascript
const { Config, QuoteContext, OAuth, Period, AdjustType, TradeSessions } = require('longport')
...
```rust
use std::sync::Arc;
use longport::{oauth::OAuthBuilder, quote::QuoteContext, Config, quote::{Period, AdjustType, TradeSessions}};
