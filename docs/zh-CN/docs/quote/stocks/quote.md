...
该接口用于获取标的的实时行情 (支持所有类型标的）。如需查看这些实时数据流汇聚而成的实时指数、板块热力图与宏观市场概览，可参考 ["全球市场"](https://longport.com/markets)。
...
或者设置环境变量 `LONGPORT_ENABLE_OVERNIGHT` 为 `true`（兼容旧版 `LONGPORT_ENABLE_OVERNIGHT`）。
