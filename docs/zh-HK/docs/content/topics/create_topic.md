...
在 [社區](https://longport.com/topics)創建一篇新討論。支持兩種內容類型：
...
僅限 **LongPort 開戶且持有資產** 的用戶才允許通過 LongPort Developers 的 API 或 CLI 發布社區討論和回覆。否則返回 `403`。
...
<CliCommand>
# 發佈 Tesla 相關話題
longport topic create --body "Tesla Q1 財報分析" --tickers TSLA.US
# 發佈 Apple 相關話題
...
# 短帖 + 關聯標的
longport topic create --body "NVDA GTC 看點" --tickers NVDA.US,700.HK

# 長文 — 支持 Markdown，必須填寫標題
longport topic create --title "我的分析" --body "**看好** 700.HK，因為..." --type article

# 長文 — 從 Markdown 文件讀取正文
longport topic create --title "Q4 財報前瞻" --body "$(cat analysis.md)" --type article

# JSON 輸出
longport topic create --body "測試帖" --format json
```
...
```python
from longport.openapi import ContentContext, Config, OAuthBuilder
...
```python
import asyncio
from longport.openapi import AsyncContentContext, Config, OAuthBuilder
...
```javascript
const { Config, ContentContext, OAuth } = require('longport')
...
```rust
use std::sync::Arc;
use longport::{oauth::OAuthBuilder, content::{ContentContext, CreateTopicOptions}, Config};
...
using namespace longport::content;
