...
- MCP 服務同步更新 — 新增 `macrodata_indicators` 和 `macrodata` 工具，可供 AI Agent 透過 LongPort MCP 服務調用
...
- **修復：Unix 自更新** — `longport update` 在 Unix 上不再出現 ETXTBUSY 錯誤
...
- 所有 API 請求統一設定 `User-Agent: longport-cli/<version>` 並新增 `x-cli-cmd` 請求標頭
...
- **`auth` 子命令群組** — `longport auth login` / `auth logout` / `auth status`；`auth status` 本機查看 Token 有效性和帳戶資訊，無需網路
...
- **CN 區域登入** — `longport auth login` 支援中國大陸區域路由
- **`-v` 標誌** — 快速查看版本號

### CLI v0.14.0

- **Device Auth** — LongPort Developers 平台現已支援 OAuth Device Auth 授權流程；`longport auth login` 顯示驗證 URL 和 Code，可在任意裝置完成授權，支援 SSH 和無頭環境
