---
slug: /quote/security/security_list
title: Overnight Eligible Securities
language_tabs: false
toc_footers: []
includes: []
search: true
...
```python
from longport.openapi import QuoteContext, Config, Market, SecurityListCategory, OAuthBuilder
...
```python
import asyncio
from longport.openapi import AsyncQuoteContext, Config, Market, SecurityListCategory, OAuthBuilder
...
```javascript
const { Config, QuoteContext, OAuth, SecurityListCategory } = require('longport')
...
```rust
use std::sync::Arc;
use longport::{oauth::OAuthBuilder, quote::QuoteContext, Config, quote::SecurityListCategory};
