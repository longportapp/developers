...
- MCP server updated in sync — `macrodata_indicators` and `macrodata` tools now available to AI agents via the LongPort MCP server
...
- **Fix: Unix self-update** — `longport update` no longer fails with ETXTBUSY on Unix
...
- Sets `User-Agent: longport-cli/<version>` and adds `x-cli-cmd` header on all API requests
...
- **`auth` subcommand group** — `longport auth login` / `auth logout` / `auth status`; new `auth status` shows token validity, expiry, and account info locally without network
...
- **CN region login** — `longport auth login` now supports China region routing
- **`-v` flag** — quick version check

### CLI v0.14.0

- **Device auth** — the LongPort Developers platform now supports OAuth Device Authorization Flow; `longport auth login` displays a verification URL and code to authorize from any device, including SSH and headless environments
