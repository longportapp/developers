...
resp = ctx.finance_calendar(CalendarCategory.Ipo, "2026-01-01", "2026-12-31")
print(resp)
```
...
async def main() -> None:
    ...
    resp = await ctx.finance_calendar(CalendarCategory.Ipo, "2026-01-01", "2026-12-31")
    print(resp)
...
async function main() {
  ...
  const resp = await ctx.finance_calendar(CalendarCategory.Ipo, "2026-01-01", "2026-12-31")
  console.log(resp)
}
...
class Main {
    public static void main(String[] args) throws Exception {
        try (OAuth oauth = new OAuthBuilder("your-client-id").build(url -> System.out.println("Open to authorize: " + url)).get();
            ...
            var resp = ctx.getFinanceCalendar(CalendarCategory.Ipo, "2026-01-01", "2026-12-31").get();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resp = ctx.finance_calendar(CalendarCategory.Ipo, "2026-01-01", "2026-12-31").await?;
    println!("{:?}", resp);
    Ok(())
}
```
...
using namespace longport::calendar;
...
int main() {
    OAuthBuilder("your-client-id").build(
        ...
        [](auto res) {
            ...
            CalendarContext ctx = CalendarContext::create(config);
            ctx.finance_calendar(CalendarCategory.Ipo, "2026-01-01", "2026-12-31") {
...
import (
	...
	"github.com/longportapp/openapi-go/calendar"
)
...
func main() {
	...
	defer c.Close()
	resp, err := c.FinanceCalendar(context.Background(), "Ipo", "2026-01-01", "2026-12-31")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%+v\n", resp)
