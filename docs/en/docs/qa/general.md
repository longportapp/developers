...
## Q1: Do I need to open a live account to call LongPort Developers?
...
Please visit [Development Center](https://open.longport.com/dashboard/) to enable the paper account and obtain the corresponding App Key & Secret and Access Token.
...
## Q7: Are there additional charges for trading operations through LongPort Developers?
...
You can set the environment variable `LONGPORT_PRINT_QUOTE_PACKAGES` to `false` (or legacy `LONGPORT_PRINT_QUOTE_PACKAGES`), or set `enable_print_quote_packages` to `false` when creating the `Config` object in the code to disable the permission table output in the console.
